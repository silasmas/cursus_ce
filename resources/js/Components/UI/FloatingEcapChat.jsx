import { useState } from 'react';
import EcapWhatsAppChat from '../Ecap/EcapWhatsAppChat';

/**
 * Bouton flottant — ouvre le chat ECAP style WhatsApp (acteurs pré-listés).
 *
 * @param {Object} props Props chat
 * @returns {JSX.Element|null}
 */
export default function FloatingEcapChat({
  enabled,
  contacts = [],
  contactsEmpty = false,
  initialPeerId = null,
  initialMessages = [],
  pollUrl,
  sendUrl,
  unreadCount = 0,
  unreadUrl = null,
  pulse = false,
}) {
  const [open, setOpen] = useState(false);

  if (!enabled) {
    return null;
  }

  const markAllRead = async () => {
    if (!unreadUrl || unreadCount === 0) {
      return;
    }

    await fetch(unreadUrl.replace('/unread', '/lu'), {
      method: 'POST',
      headers: {
        Accept: 'application/json',
        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]')?.content ?? '',
        'X-Requested-With': 'XMLHttpRequest',
      },
      credentials: 'same-origin',
    });

    window.location.reload();
  };

  return (
    <>
      {!open && (
        <div className="fixed bottom-6 right-6 z-[100] flex flex-col items-end gap-2">
          {unreadCount > 0 && unreadUrl && (
            <button
              type="button"
              onClick={markAllRead}
              className="rounded-full border border-phila-gray-200 bg-white px-3 py-1.5 text-[10px] font-semibold text-phila-gray-700 shadow-sm hover:bg-phila-gray-50"
            >
              Tout marquer lu ({unreadCount})
            </button>
          )}
          <button
            type="button"
            onClick={() => setOpen(true)}
            className={`relative flex h-14 w-14 items-center justify-center rounded-full bg-phila-orange text-2xl text-white shadow-lg transition hover:scale-105 hover:bg-phila-orange-hover ${
              pulse || unreadCount > 0 ? 'animate-ecap-chat-pulse ring-4 ring-red-500/40' : ''
            }`}
            aria-label="Ouvrir les messages ECAP"
          >
            💬
            {unreadCount > 0 && (
              <span className="absolute -right-1 -top-1 flex h-5 min-w-[20px] animate-bounce items-center justify-center rounded-full bg-red-600 px-1 text-[10px] font-bold text-white">
                {unreadCount > 99 ? '99+' : unreadCount}
              </span>
            )}
          </button>
        </div>
      )}

      {open && (
        <div
          className="fixed inset-0 z-[110] flex items-center justify-center bg-black/40 p-3 sm:p-6"
          role="dialog"
          aria-modal="true"
        >
          <div className="flex h-[min(90vh,720px)] w-full max-w-4xl flex-col">
            <EcapWhatsAppChat
              contacts={contacts}
              contactsEmpty={contactsEmpty}
              initialPeerId={initialPeerId}
              initialMessages={initialMessages}
              pollUrl={pollUrl}
              sendUrl={sendUrl}
              title="Messages ECAP"
              subtitle="Superviseur · Modérateur"
              peerSubtitle="Acteur de vacation"
              onClose={() => setOpen(false)}
            />
          </div>
        </div>
      )}
    </>
  );
}
