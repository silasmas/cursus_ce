...
@php
    ...
    $navigationHelp = $attributes->get('data-navigation-help');
    $itemAttributes = $attributes->except('data-navigation-help');
@endphp
...
<li
    {{
        $itemAttributes->class([
            ...
            'fi-sidebar-item-has-help' => filled($navigationHelp),
        ])
    }}
>
    <div class="fi-sidebar-item-row">
        ...
        @if (filled($navigationHelp))
            <button
                type="button"
                class="fi-sidebar-item-help-btn"
                x-data
                x-tooltip="{
                    content: @js($navigationHelp),
                    placement: document.dir === 'rtl' ? 'left' : 'right',
                    theme: $store.theme,
                }"
                @if ($sidebarCollapsible && (! $subNavigation))
                    x-show="$store.sidebar.isOpen"
                @endif
                aria-label="Information sur ce menu"
            >
                {{ \Filament\Support\generate_icon_html(\Filament\Support\Icons\Heroicon::OutlinedInformationCircle, attributes: (new \Illuminate\View\ComponentAttributeBag)->class(['fi-sidebar-item-help-icon']), size: \Filament\Support\Enums\IconSize::Small) }}
            </button>
        @endif
    </div>
